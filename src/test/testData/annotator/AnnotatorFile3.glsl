...
void main() {
    ...
    vec3 <error descr="Incompatible types in initialization (and no available implicit conversion).Required: vec3; Found: vec4">b</error> = a[0];
    vec3 <error descr="Incompatible types in initialization (and no available implicit conversion).Required: vec3; Found: vec4">b</error> = a[0][0];
